...
use crate::jobs::SequencedJobEvent;
...
pub(crate) async fn job_events(
    ...
    let (mut initial, rx) = state.jobs.subscribe(&job_id).await?;
    if let Some(last_event_sequence) = last_event_sequence(&headers) {
        initial.retain(|event| event.sequence > last_event_sequence);
    }
...
pub(crate) async fn server_events(
    State(state): State<ServerState>,
) -> Sse<impl Stream<Item = Result<Event, Infallible>>> {
    let rx = state.jobs.subscribe_server_events();
    let stream = futures::stream::unfold(rx, |mut rx| async move {
        loop {
            match rx.recv().await {
                Ok(event) => return Some((Ok(server_sse_event(event)), rx)),
                Err(broadcast::error::RecvError::Lagged(skipped)) => {
                    tracing::warn!(skipped, "global event subscriber lagged");
                    continue;
                }
                Err(broadcast::error::RecvError::Closed) => return None,
            }
...
fn sse_event(event: SequencedJobEvent) -> Event {
    let SequencedJobEvent { sequence, event } = event;
...
fn last_event_sequence(headers: &HeaderMap) -> Option<u64> {
    let value = headers.get("last-event-id")?;
    match value.to_str().ok().and_then(|value| value.parse().ok()) {
        Some(sequence) => Some(sequence),
        None => {
            tracing::warn!(?value, "ignoring an invalid Last-Event-ID header");
            None
        }
    }
}

fn server_sse_event(event: events::ServerEvent) -> Event {
    let event_name = match &event {
        events::ServerEvent::InvalidateAll => "invalidate-all",
    };
    Event::default().event(event_name).data(
        serde_json::to_string(&event)
            .assert_ok("a Sidex-generated server event must serialize to JSON"),
    )
}
